<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">Crear Coche</h2></x-slot>
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        @include('gees.partials.mensajes')

        <form method="POST" action="{{ route('coches.store') }}" class="space-y-3 bg-white p-6 rounded shadow">
            @csrf
            <div>
                <input name="nombre_conductor" value="{{ old('nombre_conductor') }}" class="w-full rounded border" placeholder="Nombre del conductor">
                <x-input-error :messages="$errors->get('nombre_conductor')" class="mt-2" />
            </div>
            <div>
                <input name="numero_plazas" type="number" value="{{ old('numero_plazas') }}" class="w-full rounded border" placeholder="Número de plazas">
