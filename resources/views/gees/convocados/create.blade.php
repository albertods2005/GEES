<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">Crear Convocado</h2></x-slot>
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        @include('gees.partials.mensajes')

        <form method="POST" action="{{ route('convocados.store') }}" class="space-y-3 bg-white p-6 rounded shadow">
            @csrf
            <label class="block">Partido
                <select name="id_partido" class="w-full rounded border">
                    @foreach($partidos as $partido)
                        <option value="{{ $partido->id_partido }}" @selected(old('id_partido') == $partido->id_partido)>Partido #{{ $partido->id_partido }} - {{ $partido->equipo_rival }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <input name="dorsal" type="number" value="{{ old('dorsal') }}" class="w-full rounded border" placeholder="Dorsal">
