<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">Crear Relación Usuario/Equipo</h2></x-slot>
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        @include('gees.partials.mensajes')

        <form method="POST" action="{{ route('usuarios-equipos.store') }}" class="space-y-3 bg-white p-6 rounded shadow">
            @csrf
            <label class="block">Usuario
                <select name="id_usuario" class="w-full rounded border">
                    @foreach($usuarios as $usuario)
                        <option value="{{ $usuario->id_usuario }}" @selected(old('id_usuario') == $usuario->id_usuario)>{{ $usuario->nombre_usuario }}</option>
                    @endforeach
                </select>
            ...
            <label class="block">Rol
                <select name="rol" class="w-full rounded border">
                    ...
                    <option value="familiar" @selected(old('rol') == 'familiar')>familiar</option>
                </select>
