<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">Crear Partido</h2></x-slot>
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        @include('gees.partials.mensajes')

        <form method="POST" action="{{ route('partidos.store') }}" class="space-y-3 bg-white p-6 rounded shadow">
            @csrf
            ...
            <div>
                <input name="equipo_rival" value="{{ old('equipo_rival') }}" class="w-full rounded border" placeholder="Equipo rival">
                <x-input-error :messages="$errors->get('equipo_rival')" class="mt-2" />
            </div>
            <div>
                <input type="date" name="fecha" value="{{ old('fecha') }}" class="w-full rounded border">
            ...
            <div>
                <input type="time" name="hora_partido" value="{{ old('hora_partido') }}" class="w-full rounded border">
